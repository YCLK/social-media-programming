...
print("딥러닝 모델의 결정계수 :", r2_score(y, y_pred))
print("딥러닝 모델의 MSE :", mean_squared_error(y, y_pred))

print("[스크린타임 7시간 | 스트레스 7 | 수면의 질 2 → 행복도 예측]",
      (model.predict(np.array([[7, 7, 2]])))[0][0])
print("[스크린타임 2시간 | 스트레스 2 | 수면의 질 7 → 행복도 예측]",
      (model.predict(np.array([[2, 2, 7]])))[0][0])
